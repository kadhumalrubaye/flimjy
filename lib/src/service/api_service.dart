...
class ApiService {
  static String lang = '&language=ar-sa';
  final Dio _dio = Dio();

  final String baseUrl = 'https://api.themoviedb.org/3';
  final String apiKey = 'api_key=eddf7d5d90d07f7fa21d00242df56a7c';
  ...
  Future<List<SeriesModel>> getSeriesList() async {
    print('get top movies executed');
    try {
      final url = '$baseUrl/tv/popular?$apiKey$lang';
      final response = await _dio.get(url);
      print(response.data);

      var series = response.data['results'] as List;
      List<SeriesModel> seriesList =
          series.map((m) => SeriesModel.fromJson(m)).toList();
      return seriesList;
    } catch (error, stacktrace) {
      throw Exception(
          'Exception accoured: $error with stacktrace: $stacktrace');
  ...
  Future<List<Movie>> getUpComingMovies() async {
    print('upcoming movies executed');
    try {
  ...
  Future<List<Movie>> getMovies() async {
    print('upcoming movies executed');
    try {
      final url = '$baseUrl/discover/movie?$apiKey$lang';
      final response = await _dio.get(url);
  ...
  Future<List<Movie>> getMovieByGenre(int movieId) async {
    print('getMovieByGenre movies executed');
    try {
  ...
  Future<List<Genre>> getGenreList() async {
    try {
      final response = await _dio.get('$baseUrl/genre/movie/list?$apiKey$lang');
  ...
  Future<MovieDetail> getMovieArabicDetail(int movieId) async {
    try {
      final response = await _dio.get('$baseUrl/movie/$movieId?$apiKey$lang');
      MovieDetail movieDetail = MovieDetail.fromJson(response.data);
